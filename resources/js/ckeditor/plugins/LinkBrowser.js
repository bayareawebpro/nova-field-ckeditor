...
export default class LinkBrowser{
    ...
    createButton(locale){
        ...
        view.set('isVisible', this.config.get('linkBrowser'))
        view.on('execute', ()=>{
            this.setSelection()
            Nova.$emit(`ckeditor:link:${field}`)
        })
        Nova.$on(`ckeditor:link:${field}:write`, this.writeContent.bind(this))
        return view
    }
    ...
    //     this.model.change(writer => {
    //         writer.insertText(content, { bold: false }, this.model.document.selection.getLastPosition() );
    //     })
    // }
